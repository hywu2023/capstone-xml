...
public class MyDBHelper extends SQLiteOpenHelper {
    public MyDBHelper(Context context) {
        super(context, "NowMed.db", null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE medi (mediId INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, mediList TEXT, mediName TEXT NOT NULL, " +
    ...
    public ArrayList<ListItem> allListItems(){
        ...
        while (cursor.moveToNext() && cursor2.moveToNext()) {
            ...
            listItem.setMediList(cursor.getString(1)); // 약 리스트 8-20
            listItem.setMediName(cursor.getString(2));
            listItem.setStartDate(cursor.getString(3));
            listItem.setEndDate(cursor.getString(4));
            listItem.setTimesPerDay(cursor.getInt(5));
            listItem.setMon(cursor.getInt(6));
            listItem.setTue(cursor.getInt(7));
            listItem.setWed(cursor.getInt(8));
            listItem.setThu(cursor.getInt(9));
            listItem.setFri(cursor.getInt(10));
            listItem.setSat(cursor.getInt(11));
            listItem.setSun(cursor.getInt(12));
    ...
    //EnedItem
    public ArrayList<EndedItem> allEndedItems(){
        ArrayList<EndedItem> endedItems = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor, cursor2;
        cursor = db.rawQuery("SELECT * FROM medi WHERE endDate >= date('now');", null);
        cursor2 = db.rawQuery("SELECT * FROM medi JOIN time ON medi.mediId = time.timeId WHERE endDate >= date('now');", null);
        ...
        while (cursor.moveToNext() && cursor2.moveToNext()) {
            EndedItem endedItem = new EndedItem();
            endedItem.setMediId(cursor.getInt(0));
            endedItem.setMediList(cursor.getString(1)); // 약 리스트 8-20
            endedItem.setMediName(cursor.getString(2));
            endedItem.setStartDate(cursor.getString(3));
            endedItem.setEndDate(cursor.getString(4));
            endedItem.setTimesPerDay(cursor.getInt(5));
            endedItem.setMon(cursor.getInt(6));
            endedItem.setTue(cursor.getInt(7));
            endedItem.setWed(cursor.getInt(8));
            endedItem.setThu(cursor.getInt(9));
            endedItem.setFri(cursor.getInt(10));
            endedItem.setSat(cursor.getInt(11));
            endedItem.setSun(cursor.getInt(12));
            endedItem.setOneTime(cursor2.getString(14));
            endedItem.setTwoTime(cursor2.getString(15));
            endedItem.setThreeTime(cursor2.getString(16));
            endedItems.add(endedItem);
        }
